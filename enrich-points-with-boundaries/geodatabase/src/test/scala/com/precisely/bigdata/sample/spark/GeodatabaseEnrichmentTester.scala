...
class GeodatabaseEnrichmentTester extends FunSuite with BeforeAndAfterAll {
  ...
  test("Execute geodatabase sample and assert output") {
    val paths = new Array[String](4)

    paths(0) = "../data/CrimeIndex_Sample/CrimeIndex.gdb"
    ...
    GeodatabaseEnrichment.main(paths)

    VerificationDriver.main(Array(paths(2)))
  }

  def testDriver(tableDir : String, mainMethodExecution : Array[String] => Unit) : Unit = {
  }
}
