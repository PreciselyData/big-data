...
import com.precisely.bigdata.li.spark.app.drivers.pointinpolygon.DataFormat.DataFormat
...
class BaseCommandLine(arguments: Seq[String], driverClass: String, inputFieldsHelpAddition: String) extends ScallopConf(arguments) with Serialization {
  ...
  val latitudeColumnName: ScallopOption[String] = opt[String](descr = "Latitude column name", required = true, noshort = true)
  val longitudeColumnName: ScallopOption[String] = opt[String](descr = "Longitude column name", required = true, noshort = true)
  ...
  val libraries: ScallopOption[String] = opt[String](descr = "Libraries in case of geodatabase table-file-type parameter", required = false, noshort = true)
